  ...
  const pay = async () => {
    ...
    try {
      ...
      const json = (await res.json()) as {
        mode?: string; mockUrl?: string; paymentSessionId?: string; cashfreeMode?: string; error?: string;
      };
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 980, margin: "0 auto", padding: "32px 22px 70px" }}>
        ...
        <p className="muted" style={{ marginTop: 6, fontSize: 15.5 }}>Top up any amount — <strong>₹1 = 1 credit</strong>. Each issued certificate uses one credit. Verifying is always free, forever.</p>

        {status === "success" && (
          <div className="card" style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 18px", marginTop: 18, background: "var(--green-tint)", borderColor: "var(--green)" }}>
            <Icon name="check" size={20} style={{ color: "var(--green-700)" }} /> <span style={{ fontWeight: 600, color: "var(--green-700)" }}>Payment successful — credits added to your account.</span>
          </div>
        )}
        {status === "error" && (
          <div className="card" style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 18px", marginTop: 18, background: "var(--danger-tint)", borderColor: "var(--danger)" }}>
            <Icon name="x" size={20} style={{ color: "var(--danger)" }} /> <span style={{ fontWeight: 600, color: "var(--danger)" }}>Payment could not be completed. No credits were added.</span>
          </div>
        )}
        ...
        <div className="prev-grid" style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 24, marginTop: 24, alignItems: "start" }}>
          <div className="card" style={{ padding: 26 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
              {QUICK.map((q) => (
                <button key={q} onClick={() => setAmount(q)} style={{ padding: "8px 16px", borderRadius: 99, fontSize: 14, fontWeight: 700, border: `1.5px solid ${amount === q ? "var(--green)" : "var(--line-2)"}`, background: amount === q ? "var(--green-tint)" : "#fff", color: amount === q ? "var(--green-700)" : "var(--ink-2)" }}>₹{q}</button>
              ))}
            </div>
            ...
            <div className="field">
              ...
              <div style={{ position: "relative" }}>
                <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", fontWeight: 700, color: "var(--ink-3)" }}>₹</span>
                <input className="input" type="number" min={MIN} value={Number.isFinite(amount) ? amount : ""} onChange={(e) => setAmount(parseInt(e.target.value, 10))} style={{ paddingLeft: 30, height: 52, fontSize: 18, fontWeight: 700 }} />
              </div>
              <p className="muted" style={{ fontSize: 12.5 }}>Minimum ₹{MIN}. You&apos;ll receive <strong>{valid ? amount : 0}</strong> credits.</p>
            </div>

            {error && <div style={{ fontSize: 13.5, color: "var(--danger)", background: "var(--danger-tint)", padding: "10px 12px", borderRadius: 8, marginTop: 6 }}>{error}</div>}

            <button className="btn btn-primary btn-block btn-lg" style={{ marginTop: 18 }} onClick={pay} disabled={loading || !valid}>
              <Icon name="coins" size={18} /> {loading ? "Starting…" : `Pay ₹${valid ? amount : MIN} with Cashfree`}
            </button>
            {!configured && (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div className="card" style={{ padding: 20, background: "linear-gradient(135deg,#0F1B2D,#1d2f48)", color: "#fff", border: "none" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}><Seal size={40} drawCheck={false} /><span style={{ fontWeight: 700 }}>{plan === "pro" ? "Pro plan" : "Free plan"}</span></div>
